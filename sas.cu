...
double* createFilter(int row, int column, double sigmaIn)
{
	...
	for (int x = -row / 2; x <= row / 2; x++)
	{
		...
		{
			coordSum = (x*x + y*y);
			filter[x + row / 2][y + column / 2] = (exp(-(coordSum) / constant)) / (MYPIE * constant);
		...
		if (xIndex + 3 < rows && yIndex + 3 < cols) {
			for (int x = 0; x < 3; x++)
				for (int y = 0; y < 3; y++)
				{
					...
					sumx += xFilter[fIndex] * (imgIn[index]); 
					sumy += yFilter[fIndex] * (imgIn[index]); 
				}
